"""
pages/Marktvergleich.py — Globaler Indexvergleich (2021 – heute)

Zeigt normierte oder absolute Performance der wichtigsten Weltindizes.
ETF-Proxies werden verwendet, wo kein direkter yfinance-Ticker existiert.
"""
...
def download_prices(tickers: tuple[str, ...], start: str, end: str) -> pd.DataFrame:
    ...
    df = pd.concat(frames, axis=1, sort=True).ffill()
    return df
...
with st.sidebar:
    ...
    mode = st.radio("Kursmodus", ["Normiert (Basis 100)", "Absolut"],
                    horizontal=True)
    show_event = st.toggle("📍 Kriegsbeginn einblenden (24.02.2022)", value=True)
    ...
    for region in REGIONS:
        ...
        region_indices = [n for n, v in INDICES.items() if v["region"] == region]
        for name in region_indices:
            checked = st.checkbox(name, value=True, key=f"cb_{name}")
            if checked:
                selected_names.append(name)

    st.divider()
    st.caption("Fehlende Indizes werden als ETF-Proxies geladen.\nDaten: Yahoo Finance / yfinance.")
...
for name in selected_names:
    t = INDICES[name]["ticker"]
    if t in prices_raw.columns and prices_raw[t].dropna().size > 10:
        available.append(name)

missing = [n for n in selected_names if n not in available]

prices = prices_raw[[INDICES[n]["ticker"] for n in available]]
...
if missing:
    st.warning(
        f"Folgende Indizes konnten nicht geladen werden (yfinance-Problem): "
        f"{', '.join(missing)}"
    )
...
# War annotation
if show_event and EVENT_DATE >= pd.Timestamp(start_date) <= pd.Timestamp(end_date):
    fig.add_vline(
        x=EVENT_DATE,
...
for name in available:
    ...
    # Performance seit Kriegsbeginn (falls im Zeitraum)
    if EVENT_DATE in col.index:
        perf_war = col.iloc[-1] / col.loc[EVENT_DATE] - 1
    else:
    ...
    rows.append({
        ...
        "Proxy":                        "ETF" if INDICES[name]["note"].startswith("ETF") else "direkt",
    })
